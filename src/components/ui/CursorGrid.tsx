...
export const CursorGrid = ({
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      // Continuous Ambient Wave Sweep Loop
      if (p.autoLoop) {
        ambientTime += dt * 0.0018;
        const ambX = (w / 2) + Math.sin(ambientTime * 1.5) * (w * 0.42);
        const ambY = (h / 2) + Math.cos(ambientTime * 1.1) * (h * 0.38);
        energize(ambX, ambY, 0.95);

        // Periodically emit an organic pulse ring every ~2.6 seconds
        if (p.clickPulse && now - lastPulseTime > 2600) {
          lastPulseTime = now;
          pulses.push({ x: ambX, y: ambY, t0: now });
        }
      }
      ...
      let anyVisible = pulses.length > 0 || p.autoLoop;
  ...
  useEffect(() => {
    wakeRef.current?.();
  }, [gridOpacity, color, lineWidth, maxOpacity, fillOpacity, cellRadius, autoLoop]);

  return (
    <div ref={containerRef} className={`cursor-grid absolute inset-0 pointer-events-none z-0 overflow-hidden${className ? ` ${className}` : ''}`}>
